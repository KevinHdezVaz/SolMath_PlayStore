...
package com.solumath.ncalc.geom2d.line;

import android.graphics.Path;

import com.solumath.ncalc.geom2d.util.Angle2D;
import com.solumath.ncalc.geom2d.util.EqualUtils;
import com.solumath.ncalc.geom2d.GeometricObject2D;
...
import com.solumath.ncalc.geom2d.polygon.LinearCurve2D;
...
public class LineSegment2D extends AbstractLine2D implements Cloneable {
    ...
    public static double getEdgeAngle(LineSegment2D edge1, LineSegment2D edge2) {
        ...
        if (Math.abs(edge1.x0 - edge2.x0) < Shape2D.ACCURACY
                && Math.abs(edge1.y0 - edge2.y0) < ACCURACY) {
            x0 = edge1.x0;
            y0 = edge1.y0;
        ...
        } else if (Math.abs(edge1.x0 + edge1.dx - edge2.x0) < ACCURACY
                && Math.abs(edge1.y0 + edge1.dy - edge2.y0) < ACCURACY) {
            ...
            x2 = edge2.x0 + edge2.dx;
            y2 = edge2.y0 + edge2.dy;
        } else if (Math.abs(edge1.x0 + edge1.dx - edge2.x0 - edge2.dx) < ACCURACY
                && Math.abs(edge1.y0 + edge1.dy - edge2.y0 - edge2.dy) < ACCURACY) {
            x0 = edge1.x0 + edge1.dx;
            y0 = edge1.y0 + edge1.dy;
            x1 = edge1.x0;
            y1 = edge1.y0;
            x2 = edge2.x0;
            y2 = edge2.y0;
        } else if (Math.abs(edge1.x0 - edge2.x0 - edge2.dx) < ACCURACY
                && Math.abs(edge1.y0 - edge2.y0 - edge2.dy) < ACCURACY) {
            x0 = edge1.x0;
            y0 = edge1.y0;
            x1 = edge1.x0 + edge1.dx;
            y1 = edge1.y0 + edge1.dy;
